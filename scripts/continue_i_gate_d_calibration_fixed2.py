...
def _registered_scores(freeze: dict[str, Any], checkpoint_scores: Path) -> Path:
    ...
    scope_path = Path("docs/i06_calibration_registered_scoring_scope_v2.json")
    scope = {
        ...
        "short_segment_policy": "mark-only-observed-min-prefix-length-error-inapplicable",
    }
    stage._scope_once(scope_path, scope)
    approval_path, _ = _approval("score-registered", "i06_calibration_registered_scoring_v2", scope_path)
    _record_registered_short_segment_correction(scope_path, approval_path, output_path)
    if not result_path.exists():
