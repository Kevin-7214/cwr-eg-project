...
def _registered_scores(freeze: dict[str, Any], checkpoint_scores: Path) -> Path:
    ...
    scope_path = Path("docs/i06_calibration_registered_scoring_scope_v4.json")
    scope = {
        ...
        "short_segment_policy": "family-minimum-token-precheck-plus-observed-kgw-min-prefix-inapplicable",
    }
    stage._scope_once(scope_path, scope)
    approval_path, _ = _approval("score-registered", "i06_calibration_registered_scoring_v4", scope_path)
    _record_registered_family_minimum_correction(scope_path, approval_path, output_path)
    if not result_path.exists():
...
def _record_registered_family_minimum_correction(
    scope_path: Path, approval_path: Path, output_path: Path
) -> None:
    correction_path = Path("manifests/i06_registered_family_minimum_correction_03.json")
    partial_path = output_path.with_suffix(output_path.suffix + ".partial")
    partial_rows = read_jsonl(partial_path) if partial_path.exists() else []
    previous_correction = Path("manifests/i06_registered_zero_token_correction_02.json")
    payload = {
        "manifest_version": "i06-registered-family-minimum-correction-v1",
        "created_at": ops._now(),
        "task_id": "I-06-registered-family-minimum-correction",
        "trigger": {
        ...
        "root_cause": (
            "The prior zero-token precheck guarded only detector_token_count == 0. MarkLLM's "
            "Unbiased detector drops the final token before scoring (input_ids[..., :-1]), so a "
            "refined character interval that tokenizes to exactly one token reached Qwen2 with "
            "sequence length zero and failed during attention projection reshape. Intervals below "
            "a family-specific minimum must be treated as explicit detector non-applicability "
            "before any MarkLLM call, not only the zero-token case."
        ),
        "correction": {
            "behavior": (
                "Before registered detector calls, tokenize the exact candidate segment with the "
                "detector tokenizer and add_special_tokens=False. Compare the count against a "
                "per-family minimum derived from the pinned MarkLLM configs (KGW 2, Unigram 1, "
                "Unbiased 6, SynthID 5). If the count is below the family minimum, record each "
                "family/key detector test for that interval as applicability=invalid with reason "
                "zero_detector_tokens (count 0) or insufficient_detector_tokens (count 1..min-1) "
                "and do not call MarkLLM. Preserve the prior KGW min_prefix_len ValueError handling "
                "as defense-in-depth. All other exceptions re-raise."
            ),
            "previous_registered_scoring_sha256": "bdb369a69f99b076a180d2a3e00159a46bc7431787740f5a17bfff77ed44868c",
            "new_registered_scoring_sha256": "c824145676dc7b452771ab9bd5a9fb0f73bf6fa1eca1c92995bf82cc9a0b8c4a",
            ...
            "partial_documents_at_correction": len(partial_rows),
            "resume_preserves_completed_rows": True,
            "completed_rows_verified_short_segment_free": True,
        },
        "isolation_audit": {
            "train_or_dev_changed": False,
            "calibration_input_changed": False,
    ...
    if correction_path.exists():
        ...
        for key in ("root_cause", "isolation_audit"):
            if existing.get(key) != payload.get(key):
                raise RuntimeError("Existing family-minimum registered correction drifted")
        return
    stage._write_new(correction_path, payload)
