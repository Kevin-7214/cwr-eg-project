...
def _calibration_features(freeze: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    scope_path = Path("docs/i06_calibration_feature_scope.json")
    ...
    for key in ("parent_freeze_manifest_sha256", "resume_manifest_sha256"):
        scope.pop(key, None)
    stage._update_code_hashes(scope)
    stage._scope_once(scope_path, scope)
    approval_path, approval = _approval("extract-features", "i06_calibration_features", scope_path)
...
def _registered_scores(freeze: dict[str, Any], checkpoint_scores: Path) -> Path:
    ...
    scope_path = Path("docs/i06_calibration_registered_scoring_scope.json")
    scope = {
    ...
    stage._scope_once(scope_path, scope)
    approval_path, _ = _approval("score-registered", "i06_calibration_registered_scoring", scope_path)
    if not result_path.exists():
